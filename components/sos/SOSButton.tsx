...
export function SOSButton({ onActivate }: SOSButtonProps) {
  ...
  const { status, holdProgress, startArming, cancelArming, setHoldProgress } = useSOSStore();
  ...
  }, [status, holdDuration, startArming, setHoldProgress, clearHold, onActivate]);

  const isArming = status === 'arming';

  return (
    <View className="items-center" accessibilityLabel="Emergency SOS button. Press and hold to activate.">
      <AnimatedView
        style={ringStyle}
      ...
      <AnimatedView style={pulseStyle}>
        <Pressable
          ...
          {isArming && (
            <View
              className="absolute bottom-4 left-4 right-4 h-1 overflow-hidden rounded-full bg-white/20">
              <View
      ...
      <Text variant="caption" muted className="mt-6 text-center">
        Hold {sosConfig.holdDurationMs / 1000}s · then {sosConfig.countdownSeconds}s countdown
      </Text>
    </View>
  );
}
